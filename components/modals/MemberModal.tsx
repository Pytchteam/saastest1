...
const MemberModal: React.FC<MemberModalProps> = ({ userId, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const path = `users/${userId}/teamMembers`;
    try {
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-8 flex gap-3">
            ...
            >
              {isLoading ? <Loader2 className="animate-spin w-4 h-4" /> : 'Add Member'}
            </button>
          </div>
        </form>
      </div>
